...
|_ @author Sidharth Raghavan (Modified)
|_ Run on 3.10.16 (64-bit)
'''
...
BASE_DIR = 'processed_hcw'
TRAIN_YEARS = [2016, 2017, 2018, 2019, 2020]  # Training data: 2016-2020
TEST_YEARS = [2021, 2022, 2023]  # Testing data: 2021-2023
VALIDATION_YEARS = [2024]  # Final validation: 2024
...
def get_all_graph_paths():
    """Get all graph paths from the specified directory structure"""
    all_paths = {
        'train': [],
        'test': [],
        'validation': []
    ...
    # Training graphs (2016-2020)
    for year in TRAIN_YEARS:
        graph_dir = os.path.join(BASE_DIR, f'hcw_output_{year}', f'contact_networks_{year}')
        if os.path.exists(graph_dir):
            paths = glob.glob(os.path.join(graph_dir, '*.graphml'))
            all_paths['train'].extend(paths)
            print(f"Found {len(paths)} training graphs for year {year}")
    
    # Testing graphs (2021-2023)
    for year in TEST_YEARS:
        graph_dir = os.path.join(BASE_DIR, f'hcw_output_{year}', f'contact_networks_{year}')
        if os.path.exists(graph_dir):
            paths = glob.glob(os.path.join(graph_dir, '*.graphml'))
            all_paths['test'].extend(paths)
            print(f"Found {len(paths)} testing graphs for year {year}")
    
    # Validation graphs (2024)
    for year in VALIDATION_YEARS:
        graph_dir = os.path.join(BASE_DIR, f'hcw_output_{year}', f'contact_networks_{year}')
        if os.path.exists(graph_dir):
            paths = glob.glob(os.path.join(graph_dir, '*.graphml'))
            all_paths['validation'].extend(paths)
            print(f"Found {len(paths)} validation graphs for year {year}")
    
    return all_paths

# Process graphs for a given set of paths
def process_graphs(graph_paths, mdro_dict):
    """Load GraphML files and convert to PyG Data objects"""
    print(f"Processing {len(graph_paths)} graph files")
...
def main():
    ...
    # Get all graph paths according to the year-based splits
    all_graph_paths = get_all_graph_paths()
    
    # Process graphs for each split
    train_data = process_graphs(all_graph_paths['train'], mdro_dict)
    test_data = process_graphs(all_graph_paths['test'], mdro_dict)
    val_data = process_graphs(all_graph_paths['validation'], mdro_dict)
    
    # If no validation data is available, create from train data
    if not val_data:
        print("No validation data from 2024 found. Splitting train data to create validation set.")
        train_data, val_data = train_test_split(train_data, test_size=0.2, random_state=42)
    
    if not train_data:
        print("No valid training graphs to process. Exiting.")
        return
    ...
    input_channels = train_data[0].x.shape[1]
